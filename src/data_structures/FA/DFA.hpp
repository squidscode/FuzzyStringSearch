...
std::ostream& serialize(std::ostream& os, DFA<long long, V>& dt){
     ...
     *  states!
     *  - A byte at the beginning of the adjacency list indicates whether the state is
     *  a normal state (aka. a reject state), a start state, or an accept state.
     */
    ll start = dt.get_start(); ll eos = LONG_MAX; // eos is the end of state adj list.
...
std::istream& deserialize(std::istream& is, DFA<long long, V>& dt){
    ...
    while(is.get(st)){ // while we can get a state_type character,
        ...
        while((is.read((char*) &t, sizeof(ll)), t != LONG_MAX)){
